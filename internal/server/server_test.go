	...
	for _, path := range []string{"/", "/assets/markdown.js", "/assets/app.js", "/assets/icon.svg", "/assets/icon-192.png", "/assets/icon-512.png", "/assets/apple-touch-icon.png", "/manifest.webmanifest", "/sw.js"} {
		t.Run(path, func(t *testing.T) {
			...
			if path == "/assets/app.js" {
				...
				for _, marker := range []string{`"Incident state"`, `"Archive"`, `"Notification archived"`, `"Allow overrides"`, `"Lock to channel"`, `"channel-timeline-view"`, `for (const item of [...items].reverse())`, `list.scrollTop = list.scrollHeight`} {
					if !strings.Contains(string(body), marker) {
