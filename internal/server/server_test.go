	...
	for _, path := range []string{"/", "/assets/emoji.js", "/assets/markdown.js", "/assets/app.js", "/assets/icon.svg", "/assets/icon-192.png", "/assets/icon-512.png", "/assets/apple-touch-icon.png", "/manifest.webmanifest", "/sw.js"} {
		t.Run(path, func(t *testing.T) {
			...
			if path == "/sw.js" {
				...
				for _, marker := range []string{"/pwa-kit/worker.js", "PWAKitWorker.installPushHandlers", "badgeCount"} {
			...
			if path == "/assets/app.js" {
				...
				for _, marker := range []string{`"Incident state"`, `["firing", "acknowledged"].includes(value.state)`, `"Archive"`, `"Notification archived"`, `"Allow overrides"`, `"Lock to channel"`, `channelEditButton.addEventListener`, `method: editingChannelID ? "PUT" : "POST"`, `"channel-timeline-view"`, `for (const item of [...items].reverse())`, `stableRenderedNode(`, `reconcileListChildren(nodes)`, `keepTimelineAtBottom`, `pinTimelineToBottom`, `captureTimelineScrollAnchor()`, `anchorTimelineToEntry(scrollAnchor)`, `list.addEventListener("scroll", handleScroll`, `JSON.stringify(responseItems) === JSON.stringify(loadedTimelineItems)`, `new ResizeObserver(restore)`, `image.addEventListener("load", restore`, `loadedTimelineItems = loadedTimelineItems.filter(item =>`, `syncDesktopAlertVersions(data.notifications || [])`, `composerSubmit.before(readButton)`, `topbarActions.prepend(readButton)`, `notificationAlertPresentation`, `sentMessageHoldMilliseconds = 10000`, `Leaving Unread only in ${seconds}s`, `heldSentMessages.set(created.id`, `openFiringView(channel.name)`, `stateFilter.value = "firing"`, `scrollInboxIntoViewOnMobile()`, `matchMedia("(max-width: 700px)").matches`, `PWAKit.createPushClient`, `renewMissing:true`} {
					if !strings.Contains(string(body), marker) {
